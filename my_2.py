...
def convert(k: str, t: str) -> str:
    """ Функция нахождения данных про учреждения по указанному региону и типу"""
    ...
    if k in ls:
        ...
        filtered_data = [{k: row[k] for k in ['university_name', 'university_address', 'post_index']} for row in
                         universities]

        with open('address_index.csv', mode='w', encoding='cp1251', newline='') as f:
            writer = csv.DictWriter(f, fieldnames=filtered_data[0].keys())
            writer.writeheader()
            writer.writerows(filtered_data)
        return "Данные успешно записаны в файл address_index.csv!"
    else:
        return "Такого региона не существует!"


def main():
    print("Добро пожаловать в программу импортирования названия, адреса и почтового индекса учебных заведений по "
          "региону!")
    print(
        "Дан следующий список кодов регионов: 01, 05, 07, 12, 14, 18, 21, 23, 26, 32, 35, 44, 46, 48, 51, 53, 56, 59, "
        "61, 63, 65, 68, 71, 73, 74, 80, 85")
    print("Учреждения могут быть таких типов: 1 - Заведения высшего образования, 2 - Заведения профессионального ("
    ...
    while choice != "0":
        print(
            """
                0 - Выйти
                1 - Найти данные учреждений по региону и по типу
                """
        )
